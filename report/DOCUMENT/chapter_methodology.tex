...
χρησιμοποιούμε τη ρουτίνα \tl{\texttt{TriDist()}} από το πακέτο 
\tl{\textit{PQP}} \cite{larsen2014pqp} \cite{larsen1999fast} 
\cite{larsen845311l} \cite{shellshear2014fast}.  

Μια σύντομη περιγραφή της ρουτίνας είναι η ακόλουθη: 
...
Το διάνυσμα $\vec{pq}$ ορίζει μια πλάκα (\tl{slab}) (ένα επίπεδο 
...
\begin{algorithm}[h]
    ...
    \SetKwFunction{meshdist}{triangle\_mesh\_distance}
    \Indm\nonl\meshdist($P$, $Q$)\\
    \Indp
        $T_P \gets$ \trias($P$) \;
        $T_Q \gets$ \trias($Q$) \;
...
αλγόριθμος \tl{\texttt{QUICKSELECT}} \cite{hoare1961algorithm}.
...
δύο υποσύνολα με τον παραπάνω τρόπο αναμένουμε πως τα δύο \tl{AABB}
...
\begin{algorithm}[h]
    ...
    \SetKwFunction{treebuild}{tree\_build}
    \SetKwFunction{trias}{triangles}
...
\begin{algorithm}[h]
    ...
    \Indp
        ...
        \If{$aabb\_dist\_l < aabb\_dist\_r$}{
            \If{$aabb\_dist\_l < distance$}{
                \nearestsearchrecur($v.left$, $q$, $distance$)
            }
            \If{$aabb\_dist\_r < distance$}{
...
\section{Αλγόριθμοι Αναζήτησης με \tl{sKD-Tree}}
Έχοντας πλέον οργανώσει τα δεδομένα 
σε μια ιεραρχία όπως το \tl{sKD-Tree} μπορούμε 
να σχεδιάσουμε αλγορίθμους που βασίζονται στις 
ρουτίνες που περιγράψαμε προηγουμένως.

Ο πιο απλός αλγόριθμος κατασκευάζει ένα \tl{sKD-Tree}
...
\IncMargin{1.5em}
\begin{algorithm}[h]
    \caption[Απόσταση Τριγωνικών Πλεγμάτων με Ερωτήματα \tl{NN} στο \tl{sKD-Tree}]{
        \tg{
            Απόσταση Τριγωνικών Πλεγμάτων με \tl{NN Queries} σε \tl{sKD-Tree}}
        }
    ...
    \Indp
        ...
        $distance \gets \inf$ \;
        \ForEach{$p \in T_P$}{
            $result \gets \nearestsearch(tree, p, distance)$ \;
            \If{$result < distance$}{
                $distance \gets result$ \;
            }
        }
        \Return{$distance$}
...
Ο δεύτερος αλγόριθμος που προτείνουμε, χρησιμοποιεί 
δύο ιεραρχίες \tl{sKD-Tree}, μία για κάθε τριγωνικό 
πλέγμα. 
...
\begin{description}
    ...
    μικρότερη. 
    \item[γραμμές 6-7] Υπολογίζονται οι αποστάσεις των \tl{AABB}
    των παιδιών του $v$ με το \tl{AABB} της ρίζας του άλλου δέντρου.
...
\IncMargin{1.5em}
\begin{algorithm}[h]
    \caption[Απόσταση Τριγωνικών Πλεγμάτων με Δύο \tl{sKD-Tree}]{
        \tg{
            Απόσταση Τριγωνικών Πλεγμάτων με Δύο \tl{sKD-Tree}}
        }
    \label{alg:search_on_two_trees}
    \DontPrintSemicolon
    \KwIn{Two triangle meshes $P$, $Q$}
    \KwOut{Euclidean distance of $P$ and $Q$}
    \SetKwFunction{searchtwotrees}{nearest\_search\_two\_trees}
    \Indm\nonl\meshdist($P$, $Q$)\\
    \Indp
...
\IncMargin{1.5em}
\begin{algorithm}[h]
    \caption[Απόσταση Τριγωνικών Πλεγμάτων με Δύο \tl{sKD-Tree} Αναδρομή]{
        \tg{
            Απόσταση Τριγωνικών Πλεγμάτων με Δύο \tl{sKD-Tree} Αναδρομικά}
        }
    \label{alg:search_on_two_trees_recursive}
    \DontPrintSemicolon
    \KwIn{A node $v$ of the first tree, the second tree $other\_tree$, NN $distance$ so far}
    \KwOut{Changes $distance$ to be the distance of the nearest triangles}
    
    \Indm\nonl\searchtwotrees($v$, $other\_tree$, $distance$)\\
    \Indp
    \If{$v$ is leaf}{
            $result \gets $ \nearestsearch($other\_tree$, $v.triangle$, $distance$) \;
            $distance \gets \min(distance, result)$ \;
            \Return{}
    }
    ...
    \If{$aabb\_dist\_l < aabb\_dist\_r$}{
        \If{$aabb\_dist\_l < distance$}{
            \nearestsearchrecur($v.left$, $other\_tree$, $distance$)
        }
        \If{$aabb\_dist\_r < distance$}{
            \nearestsearchrecur($v.right$, $other\_tree$, $distance$)
        }
    }
    \Else{
        \If{$aabb\_dist\_r < distance$}{
...
\paragraph{Μνήμη.} 
Το \tl{sKD-Tree} είναι ένα στατικό, σχεδόν πλήρες 
...
μνήμης (\tl{allocations}) κατά την κατασκευή του δέντρου. 
Το μόνο που απαιτείται είναι μία μόνο δέσμευση μνήμης.
Επιπλέον, ένα \tl{sKD-Tree} που κατασκευάζεται από $n$ 
στοιχεία/τρίγωνα απαιτεί $2n - 1$ κόμβους. 
Η ίδια σχέση ισχύει, προφανώς, και για οποιοδήποτε 
υποδέντρο του \tl{sKD-Tree} και θα χρησιμοποιηθεί παρακάτω
για τον προσδιορισμό των δεικτών των παιδιών ενός κόμβου.

Απαριθμούμε τους κόμβους του δέντρου σύμφωνα με τη
διάσχιση \tl{Euler (pre-order traversal)} και τους τοποθετούμε 
...
οποίος δεικτοδοτεί κάθε τρίγωνο του \tl{sKD-Tree}.
...
Η ανάθεση επιτυγχάνεται με τον αλγόριθμο \texttt{\tl{QUICKSELECT}} 
...
Τέλος, για την αποθήκευση του \tl{sKD-Tree} απαιτείται:
\begin{itemize}
    ...
    \item Ένας πίνακας $data\_aabb$ με τα \tl{AABB} των τριγώνων 
    ...
    ανά κόμβο είναι ένα \tl{AABB}, αυτό του κόμβου.
\end{itemize}

\paragraph{Χρήση Πολλαπλών Νημάτων.}
Στις υλοποιήσεις μας χρησιμοποιούμε πολλαπλά νήματα (\tl{multithreading})
για την παράλληλη κατασκευή του \tl{sKD-Tree} και την παράλληλη 
εκτέλεση ερωτημάτων.
...
Επομένως, καλούμε ένα \tl{thread} για την κατασκευή του 
αριστερού παιδιού ενώ η κατασκευή του δεξιού παιδιού συνεχίζεται 
στο τρέχουν \tl{thread}.
Πριν τη συνένωση των \tl{AABB} των δύο παιδιών θα πρέπει 
το \tl{thread} που δημιουργήθηκε να ολοκληρώσει την εργασία 
του. 
Επομένως, η ροή του τρέχοντος \tl{thread} σταματά μέχρι την 
ολοκλήρωση της εργασίας του άλλου \tl{thread}.
Επιπλέον, προσθέσαμε μια συνθήκη για την αποφυγή της κλήσης 
πολλών εργασιών ανεξέλεγκτα.
Έστω $nthreads$ είναι το πλήθος των διαθέσιμων \tl{threads}
...
νήμα (\tl{thread}). 
Όμως επειδή η μεταβλητή \tl{\texttt{distance}} συνεχώς αλλάζει
όταν εντοπίζονται κοντινότεροι γείτονες, για να αποφύγουμε τη 
χρήση \tl{\textit{lock}}, το κάθε νήμα εκτελεί τις γραμμές 3-9 
του αλγορίθμου με μια τοπική μεταβλητή \tl{\texttt{local\_distance}}.
Στο τέλος, επιστρέφεται η μικρότερη από αυτές.

Ο αλγόριθμος \ref{alg:search_on_two_trees} μπορεί επίσης να 
μετατραπεί σε παράλληλο εάν κατασκευάσουμε τα δύο \tl{sKD-Tree}
